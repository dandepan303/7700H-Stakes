...
float Robot::slew(float target, float current, float maxChange) {
...
constexpr float Robot::sanitizeAngle(float angle, bool radians) {
    if (radians) return std::fmod(std::fmod(angle, 2 * M_PI) + 2 * M_PI, 2 * M_PI);
    else return std::fmod(std::fmod(angle, 360) + 360, 360);
}

float Robot::angleError(float target, float position, bool radians, AngularDirection direction) {
...
float Robot::avg(std::vector<float> values) {
    float sum = 0;
    for (float value : values) { sum += value; }
    return sum / values.size();
}

float Robot::ema(float current, float previous, float smooth) {
    return (current * smooth) + (previous * (1 - smooth));
}

float Robot::getCurvature(Pose pose, Pose other) {
    // calculate whether the pose is on the left or right side of the circle
    float side = Robot::sgn(std::sin(pose.theta) * (other.x - pose.x) - std::cos(pose.theta) * (other.y - pose.y));
