...
// global variables
Robot::OdomSensors odomSensors(nullptr, nullptr, nullptr, nullptr, nullptr); // the sensors to be used for odometry
Robot::Drivetrain drive(nullptr, nullptr, 0, 0, 0, 0); // the drivetrain to be used for odometry
Robot::Pose odomPose(0, 0, 0); // the pose of the robot
Robot::Pose odomSpeed(0, 0, 0); // the speed of the robot
Robot::Pose odomLocalSpeed(0, 0, 0); // the local speed of the robot
...
void Robot::setSensors(Robot::OdomSensors sensors, Robot::Drivetrain drivetrain) {
    odomSensors = sensors;
    drive = drivetrain;
}

Robot::Pose Robot::getPose(bool radians) {
    if (radians) return odomPose;
    else return Robot::Pose(odomPose.x, odomPose.y, radToDeg(odomPose.theta));
}

void Robot::setPose(Robot::Pose pose, bool radians) {
    if (radians) odomPose = pose;
    else odomPose = Robot::Pose(pose.x, pose.y, degToRad(pose.theta));
}

Robot::Pose Robot::getSpeed(bool radians) {
    if (radians) return odomSpeed;
    else return Robot::Pose(odomSpeed.x, odomSpeed.y, radToDeg(odomSpeed.theta));
}

Robot::Pose Robot::getLocalSpeed(bool radians) {
    if (radians) return odomLocalSpeed;
    else return Robot::Pose(odomLocalSpeed.x, odomLocalSpeed.y, radToDeg(odomLocalSpeed.theta));
}

Robot::Pose Robot::estimatePose(float time, bool radians) {
...
void Robot::update() {
    ...
    Robot::TrackingWheel* verticalWheel = nullptr;
    Robot::TrackingWheel* horizontalWheel = nullptr;
