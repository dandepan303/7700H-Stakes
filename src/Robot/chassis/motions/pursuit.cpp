 ...
 * @return std::vector<Robot::Pose> vector of points on the path
 */
std::vector<Robot::Pose> getData(const asset& path) {
    std::vector<Robot::Pose> robotPath;
    ...
    for (std::string line : dataLines) {
        Robot::infoSink()->debug("read raw line {}", stringToHex(line));
        ...
        if (pointInput.size() != 3) {
            Robot::infoSink()->error("Failed to read path file! Are you using the right format? Raw line: {}",
                                      stringToHex(line));
            break;
        }
        Robot::Pose pathPoint(0, 0);
        ...
        Robot::infoSink()->debug("read point {}", pathPoint);
    }
 ...
 */
int findClosest(Robot::Pose pose, std::vector<Robot::Pose> path) {
    int closestPoint;
 ...
 */
float circleIntersect(Robot::Pose p1, Robot::Pose p2, Robot::Pose pose, float lookaheadDist) {
    // calculations
    // uses the quadratic formula to calculate intersection points
    Robot::Pose d = p2 - p1;
    Robot::Pose f = p1 - pose;
    float a = d * d;
    float b = 2 * (f * d);
 ...
 */
Robot::Pose lookaheadPoint(Robot::Pose lastLookahead, Robot::Pose pose, std::vector<Robot::Pose> path, int closest,
                            float lookaheadDist) {
    ...
    for (int i = start; i < path.size() - 1; i++) {
        Robot::Pose lastPathPose = path.at(i);
        Robot::Pose currentPathPose = path.at(i + 1);

        float t = circleIntersect(lastPathPose, currentPathPose, pose, lookaheadDist);

        if (t != -1) {
            Robot::Pose lookahead = lastPathPose.lerp(currentPathPose, t);
            lookahead.theta = i;
            return lookahead;
        }
    }
 ...
 */
float findLookaheadCurvature(Robot::Pose pose, float heading, Robot::Pose lookahead) {
    // calculate whether the robot is on the left or right side of the circle
    float side = Robot::sgn(std::sin(heading) * (lookahead.x - pose.x) - std::cos(heading) * (lookahead.y - pose.y));
...
void Robot::Chassis::follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    ...
    std::vector<Robot::Pose> pathPoints = getData(path); // get list of path points
