...
Robot::TrackingWheel::TrackingWheel(pros::adi::Encoder* encoder, float wheelDiameter, float distance,
                                     float gearRatio) {
...
Robot::TrackingWheel::TrackingWheel(pros::Rotation* encoder, float wheelDiameter, float distance, float gearRatio) {
...
Robot::TrackingWheel::TrackingWheel(pros::MotorGroup* motors, float wheelDiameter, float distance, float rpm) {
    this->motors = motors;
...
void Robot::TrackingWheel::reset() {
...
float Robot::TrackingWheel::getDistanceTraveled() {
    if (this->encoder != nullptr) {
        ...
        return Robot::avg(distances);
    } else {
        return 0;
    }
}

float Robot::TrackingWheel::getOffset() { return this->distance; }

int Robot::TrackingWheel::getType() {
